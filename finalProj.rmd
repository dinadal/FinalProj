...
```{r}
# Sample code using ggplot2
library(ggplot2)

# Create a data frame with model names and performance metrics
model_names <- c("Random Forest", "Linear Regression", "SVM", "Gradient Boosting")
mse <- c(0.329, 0.288, 0.305, 0.233)
rmse <- c(0.574, 0.537, 0.553, 0.483)
mae <- c(0.412, 0.358, 0.364, 0.329)
model_performance <- data.frame(Model = model_names, MSE = mse, RMSE = rmse, MAE = mae)

# Plotting MSE for each model
ggplot(model_performance, aes(x = Model, y = MSE)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  labs(title = "Comparative Model Performance (MSE)", y = "MSE") +
  theme_minimal() +
